  ...
  for (z in 1:m){
    ...
    for (i in data_info$phase2_cols){
      if (i %in% data_info$numeric_cols){
        pmm_matched <- pmm(output_tensor[data_info$phase2_rows, i], 
                           output_tensor[data_info$phase1_rows, i], 
                           data_norm$data[data_info$phase2_rows, i], 5)
        output_tensor[data_info$phase1_rows, i] <- pmm_matched
      }
    }
...
normalize <- function(data, data_info, method = "none"){
  if (method == "min-max"){
    ...
    for (i in data_info$phase2_cols){
      if (i %in% data_info$numeric_cols){
        data_norm[, i] <- data_norm[, data_info$phase1_cols[i]] - data_norm[, i]
      }
    }
...
denormalize <- function(data, data_info, params, method = "none"){
  for (i in data_info$phase2_cols){
    if (i %in% data_info$numeric_cols){
      data[, i] <- data[, data_info$phase1_cols[i]] - data[, i]
    }
  }
