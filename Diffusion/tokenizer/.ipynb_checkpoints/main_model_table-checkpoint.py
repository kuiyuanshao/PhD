...
from diff_models_table import diff_MECSDI
import pandas as pd
import yaml
...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device, matched_state = 1, 
                 phase1_cols = 1, phase2_cols = 1, cont_list = 1, num_cate_list = 1):
        super().__init__()
    ...
    def calc_loss(self, observed_data, cond_mask, observed_mask, side_info):
        ...
        target_mask = torch.repeat_interleave(target_mask, self.token_dim, dim=1)
    ...
    def set_input_to_diffmodel(self, noisy_data, observed_data, cond_mask):
        cond_mask = torch.repeat_interleave(cond_mask, self.token_dim, dim=1)
        ...
        B, K, L = total_input.shape
        total_input = total_input.reshape(B, K, int(L / self.token_dim), self.token_dim)
        total_input = total_input.permute(0, 1, 3, 2)
        total_input = total_input.reshape(B, K * self.token_dim, int(L / self.token_dim))
        return total_input
    ...
    def impute(self, observed_data, cond_mask, side_info, n_samples):
        ...
        for i in range(n_samples):
            ...
            for t in range(max(self.matched_state) - 1, -1, -1):
                ...
                B, K, L = diff_input.shape
                diff_input = diff_input.reshape(B, K, int(L / self.token_dim), self.token_dim)
                diff_input = diff_input.permute(0, 1, 3, 2)
                diff_input = diff_input.reshape(B, K * self.token_dim, int(L / self.token_dim))
...
class TabCSDI(CSDI_base):
    def __init__(self, config, device, target_dim = 0, matched_state = 1, 
        ...
        super(TabCSDI, self).__init__(target_dim, config, device, matched_state, 
                                      phase1_cols, phase2_cols, cont_list, num_cate_list)
        self.config = config
        self.device = device
        self.cont_list = cont_list
        self.phase1_cols = phase1_cols
    ...
    def process_data(self, batch):
        ...
        observed_data = observed_data.reshape(B, L * C)
