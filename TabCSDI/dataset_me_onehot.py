...
def process_func(path: str, cat_list, missing_ratio=0.1, encode=True):
    ...
    masks = 1 - np.isnan(data.to_numpy())
    ...
    if encode == True:
        ...
        with open("./data_me_onehot/transformed_columns.pk", "wb") as f:
            pickle.dump([cont_list, saved_cat_dict], f)

        with open("./data_me_onehot/encoder.pk", "wb") as f:
...
class tabular_dataset(Dataset):
    def __init__(self, eval_length=111, use_index_list=None, missing_ratio=0.1, seed=0):
        ...
        processed_data_path_norm = f"./data_me_onehot/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
