...
for _,  data in tqdm(enumerate(val_loader)):
    idx += 1
    data = trainer.set_device(data) # original data
    denoised = trainer.netG.denoise(data) # denoised data after the noise model
    ...
    for t in range(sqrt_alphas_cumprod_prev.shape[0]): # linear search with early stopping
        noise = data['X'] - sqrt_alphas_cumprod_prev[t] * denoised # difference between original data and denoised data, scaled by a parameter alpha
